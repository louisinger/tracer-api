...
async function main () {
  ...
  const private_blinding_key = await wally_asset_blinding_key_to_ec_private_key(masterBlindingKey, scriptPubkey)
  console.log('private blinding key: ', masterBlindingKey)

  const publicBlindingKey = await wally_ec_public_key_from_private_key(private_blinding_key)
